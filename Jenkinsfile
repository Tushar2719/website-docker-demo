pipeline {  
    ...
    environment {  
        ...
        AWS_ACCOUNT_ID = '340257302060'  
        IMAGE_TAG = "${env.BUILD_NUMBER}"  
        IMAGE_URI = "${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/${ECR_REPO}:${IMAGE_TAG}"  
        LATEST_URI = "${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/${ECR_REPO}:latest"  
        DEPLOY_SERVER = '13.201.136.183'  
    }  
  
    stages {  
        stage('Checkout') {  
            steps {  
                git branch: 'main', url: 'https://github.com/Tushar2719/website-docker-demo.git'  
            }  
        }  
    ...
    steps {
        sh '''
            aws ecr get-login-password --region ap-south-1 | docker login --username AWS --password-stdin 340257302060.dkr.ecr.ap-south-1.amazonaws.com
        '''
    }
}
